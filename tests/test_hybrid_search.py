...
def test_hybrid_search_builds_expected_payload(mocked_server):
    module, mock_cls, mock_instance, fake_results = mocked_server
...
def test_hybrid_search_live_smoke():
    load_dotenv()

    if os.getenv("RUN_AZURE_SEARCH_INTEGRATION_TESTS") not in {"1", "true", "TRUE"}:
        pytest.skip("Set RUN_AZURE_SEARCH_INTEGRATION_TESTS=1 to run live Azure Search tests.")
